...
" Highlight snipped sequence <++>
autocmd ColorScheme * highlight SnippedSign ctermbg=green guibg=green ctermfg=black
autocmd BufWinEnter * syntax match SnippedSign /<++>/ containedin=ALL
highlight SnippedSign ctermbg=green ctermfg=black

" Sync syntax highlighting from the start of the buffer when entering a buffer window
...
nnoremap <leader>* /<C-R><C-W>\C<CR>N:grep! -R <cword> * --binary-files=without-match --exclude-dir={.git,tmp,log,_build} --exclude=tags<CR><CR>:cw<CR>
...
" Template files
command! Tempbeamer execute '-1read ~/.config/nvim/templates/beamer.tex' | set ft=tex
command! Tempc execute '-1read ~/.config/nvim/templates/skeleton.c' | set ft=c
command! Tempcpp execute '-1read ~/.config/nvim/templates/skeleton.cpp' | set ft=cpp
command! Temphtml execute '-1read ~/.config/nvim/templates/skeleton.html' | set ft=html
command! Templetter execute '-1read ~/.config/nvim/templates/letter.tex' | set ft=tex
command! Tempm execute '-1read ~/.config/nvim/templates/Makefile' | set ft=make
command! Tempsh execute '-1read ~/.config/nvim/templates/skeleton.sh' | set ft=sh
